...
INT_PTR CALLBACK searchDialogProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_COMMAND:
        ...
        case IDC_SEARCH_TOOLS:
            showToolsMenu(reinterpret_cast<HWND>(lParam));
            return TRUE;
        }
...
void syncReplaceButton() {
    if (data.dockingDialog) SetDlgItemText(data.dockingDialog, IDC_SEARCH_REPLACE, Command_Button(data.buttonReplace));
    if (data.regularDialog) SetDlgItemText(data.regularDialog, IDC_SEARCH_REPLACE, Command_Button(data.buttonReplace));
}
...
void menuCommandFind() {
    ...
    auto result = RequestSearch(data.buttonFind.get(), data.context, data.find, data.repl, plugin.currentScintilla());
    if (IsWindowVisible(data.searchDialog)) showMessage(data.searchDialog, result);
    else npp(NPPM_SETSTATUSBAR, STATUSBAR_DOC_TYPE, result.message.data());
}
...
void menuCommandReplace() {
    ...
    auto result = RequestSearch(data.buttonReplace.get(), data.context, data.find, data.repl, plugin.currentScintilla());
    if (IsWindowVisible(data.searchDialog)) showMessage(data.searchDialog, result);
    else npp(NPPM_SETSTATUSBAR, STATUSBAR_DOC_TYPE, result.message.data());
}